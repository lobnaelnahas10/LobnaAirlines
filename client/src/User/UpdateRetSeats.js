...
const UpdateRetSeats=()=>{
    ...
    useEffect(async()=>{
     ...
     let res2 = await axios.get(`http://localhost:8000/viewFlight/${res.data.return}`)
          ...
          const OCC=(array,c)=>{
            ...
            const handleInput=(e)=>{
               ...
               if(a.length<x && !a.includes(e)){
                a.push(e);
                toast(" Return Flight Seats Selected "+a);
               }
               else if(a.includes(e)){
                var y=a.indexOf(e);
                 a.splice(y,1)
                  toast("You Removed Seat"+e+" Return Flight Seats Selected ="+a);
               }
               console.log(a);
                } 
             ...
             const updateReservation=async()=>{
                ...
                console.log("s"+s);
                 let user1=user._id;
                 console.log("a"+a)
                 axios.put(`http://localhost:8000/updateReservation/${id}`,({departure:flight1,return:flight2,user:user1,price:pricefinal,cabin:cabin1,bookingNumber:bookingNumber,depSeats:depSeats,retSeats:s}),{headers:headers});
                 toast("Seats Updated");
             }
